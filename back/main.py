...
speech_key = '4643d5025f3043e1b221e2752ce7f022'
...
def azure_batch_stt(filename: str):
    ...
    words = [word for result_json in all_jsons for word in json.loads(result_json)[
        'NBest'][best_index]['Words']]
    print(words)
    ...
    sentiments = [sentiment_classifier(speech_utterance)
                  for speech_utterance in utterances]
    print(sentiments)
    ...
    if speech_length <= 20:
        ...
        summary = summarizer(full_transcript, min_length=int(
            0.1*speech_length), max_length=int(0.8*speech_length))[0]["summary_text"]
    print(summary)
...
class App:
    ...
    def __post_init__(self):
        ...
        self.server.add_url_rule(
            "/speech2text", "speech2text", self.speech2text, methods=["POST"])
        self.server.add_url_rule(
            "/", "default", self.catch, defaults={"path": ""})
        self.server.add_url_rule(
    ...
    def speech2text(self):
        ...
        # Call model
        transcript, token_times, sentiment, summary, ners, keywords = azure_batch_stt(
            "sample.wav")
